...
- **Preserve original + modern** (revised 2026-06-23 per user): keep a best-effort **verbatim 1864-orthography `text`** AND a **`modern_text`** (modernized), the same way Franko is stored. `data/sources/nomis.csv` columns: **`ref,text,modern_text`** (`text` = original reading, lightly de-OCR'd; `modern_text` = modernized; `ref` = Nomis entry № or ""). The adapter (`adapters/nomis.py`, Task 1) reads `ref`+`text` (the original) and ignores the extra `modern_text` column; the modernized value is wired into enrichment for net-new entries in Task 5 (it must NOT default to `=text`).
...
- [ ] **Step 1: Extraction prompt** (per page of OCR text) — VALIDATED on page 120 (preserves archaic forms, strips apparatus). System/task:
  > You are extracting genuine Ukrainian proverbs/приказки from OCR'd text of Номис, «Українські приказки, прислів'я і таке інше» (1864) — a two-column critical edition in 1864 orthography. The OCR contains proverbs mixed with apparatus (entry numbers, source sigla like «Йл.»/«Зв.»/«Бр.»/«Ном.», variant readings, editorial prose) and OCR noise. For each genuine proverb output `{"text":…, "modern_text":…, "ref":…}`:
  > - **`text`** = the proverb in its ORIGINAL 1864 wording. **PRESERVE archaic/dialectal wordforms and historical orthography exactly — they are NOT errors:** keep «вбувсь» (do NOT change to «обувсь»), «коровая» (do NOT change to «коровай»), endings «-цця/-єцця», «и» where modern uses «і», «ѣ», «ъ», dialectal vocabulary. ONLY remove genuine OCR artifacts: stray marks/quotes (`(")`, `//`), mojibake, and clearly misread letters (e.g. «бго»→«його», «Шіди»→«Піди»). **When unsure whether something is an archaic form or an OCR error, KEEP it as printed.**
  > - **`modern_text`** = the same proverb in modern standard Ukrainian spelling (this is where modernization belongs — «вбувсь»→«взувся», «коровая»→«короваю»).
  > - **`ref`** = the Nomis entry number if one clearly precedes it, else "".
  > Include ONLY actual proverbs/sayings. EXCLUDE entry-number-only fragments, sigla, variant-reading apparatus, commentary, OCR garbage. If a line is too garbled to recover confidently, omit it. Return ONLY a JSON array.
  >
  > NOTE (audit): the extractor preserves the OCR's *reading*, so character-level OCR errors (e.g. «коровай»/«5410» where the scan prints «коровая»/«5470») persist in `text` — caught only by the image spot-audit (Step 4), not by extraction. Best-effort.
...
- [ ] **Step 3: Aggregate** → `data/sources/nomis.csv` with header `ref,text,modern_text` (one row per extracted proverb; `text` = original 1864 wording, `modern_text` = modernized). De-duplicate exact repeats within Nomis (on `text`). Record the count.
...
- [ ] **Step 2: Re-attach enrichment** — `expand.reattach.reattach(base_rows, enriched_rows) -> (attached, new_ids)`. `attached` carries modern_text/category/explanation for all proverbs that existed before (matched on `normalized_text`); `new_ids` = the Nomis net-new entries needing categorization. **Then override `modern_text` for the Nomis net-new** from `data/sources/nomis.csv`: build a `{normalize(text): modern_text}` map from nomis.csv and set each net-new row's `modern_text` from it (reattach's default `modern_text=text` keeps the ORIGINAL — wrong now that we preserve it; the modernized form must come from the extraction). `text` stays the original 1864 reading.
