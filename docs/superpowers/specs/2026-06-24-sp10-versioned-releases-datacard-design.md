...
- **`CITATION.cff`** — Citation File Format 1.2.0 (GitHub renders a "Cite this repository" button). Title, authors, version, date-released, URL, license, keywords, type `dataset`.
...
- **Compilation + enrichment** (the unified corpus structure, `modern_text`, `category`, cleaned `explanation`, `variant_group`): **CC BY 4.0** (default — confirm at spec review).
...
- **Licensing:** §6 is the maintainer's call; the default (CC BY 4.0 compilation + PD historical + upstream-rights modern) is stated for confirmation at spec review. Getting this right matters for the corpus's reputation.
