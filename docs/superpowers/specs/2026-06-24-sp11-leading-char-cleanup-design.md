...
**Sub-project:** 11 — scrub OCR leading-junk + mixed-script homoglyphs from `text`; ship as corpus **v1.0.1**
...
| class | count | verdict |
|---|---|---|
| `«` `"` `„` quoted **dialogue** proverbs | ~455 | **preserve** (genuine) |
...
**In scope:** ~210 genuine defects (the strip + homoglyph + judgment rows). **Out of scope:** the ~464 legitimate `« „ " Ѣ` entries (untouched); a full corpus-wide mixed-script audit beyond the flagged entries (separate future work); changing the schema.
...
- An LLM pass (batched, like the original enrichment) reads each judgment/homoglyph entry and proposes the corrected `text` (and `modern_text` if it changes), with the rule: repair OCR confusables to Cyrillic, fix obvious case/leading errors, **preserve** archaic forms and genuine dialogue/quotes, and when unsure **leave unchanged** (omit the row). Output → `corrections.csv`.
...
- `core/clean.py` (NEW) — `clean_text()` + the leading-junk rules. [pytest]
...
- **Post-build assertions:** re-run `scan_leading.py` on the rebuilt corpus → the ONLY remaining non-uppercase-start entries are the legitimate `« „ " Ѣ` class (count ≈ 464); **zero** entries lost `category`/`modern_text` vs before; total count unchanged (or reduced only by intentional merges, logged).
